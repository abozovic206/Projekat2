...
namespace FitnessAppBackend2_.Controllers
{
    ...
    public class NutritionController : ControllerBase
    {
        ...
        [HttpPut("{id}")]
        public async Task<ActionResult<NutritionItem>> UpdateNutritionItem(int id, [FromForm] UpdateNutritionDTO updateNutritionDTO)
        {
            try
            {
                var updateItem = await _nutritionService.UpdateNutritionItemAsync(id, updateNutritionDTO);
